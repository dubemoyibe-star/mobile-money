...
export class KYCController {
  ...
  uploadDocument = async (req: Request, res: Response) => {
    try {
      ...
      // Verify user owns this applicant
      const hasAccess = await this.verifyApplicantAccess(userId, validatedData.applicant_id);
      if (!hasAccess) {
        return res.status(403).json({ error: 'Access denied' });
      }
  ...
  getVerificationStatus = async (req: Request, res: Response) => {
    try {
      ...
      const verificationStatus = await this.kycService.getVerificationStatus(applicantId);
  ...
  getUserKYCStatus = async (req: Request, res: Response) => {
    try {
      ...
      const transactionLimits = this.kycService.getTransactionLimits(currentKYCLevel);
  ...
  private async verifyApplicantAccess(userId: string, applicantId: string): Promise<boolean> {
    try {
      const query = `
